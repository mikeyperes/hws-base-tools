...
function hws_sanitize_site_type( string $site_type ): string {
    ...
    return array_key_exists( $site_type, hws_site_type_options() ) ? $site_type : 'other';
}

function hws_get_site_type(): string {
    return hws_sanitize_site_type( (string) get_option( HWS_SITE_TYPE_OPTION, 'other' ) );
}
...
function hws_get_site_type_label( ?string $site_type = null ): string {
    ...
    return $options[ $site_type ] ?? $options['other'];
}
