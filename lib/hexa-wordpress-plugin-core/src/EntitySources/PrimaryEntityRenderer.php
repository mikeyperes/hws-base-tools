...
document.addEventListener('change',function(event){var root=event.target.closest('[data-hpc-primary-entity]');if(!root)return;if(event.target.closest('.hpc-primary-site-type'))updateDerivedType(root);var source=event.target.closest('.hpc-primary-source');if(!source)return;root.querySelectorAll('.hpc-primary-source-search').forEach(function(item){item.hidden=item.dataset.sourceId!==source.value;});});
document.addEventListener('hexa-search-selected',function(event){var root=event.target.closest('[data-hpc-primary-entity]');if(!root)return;var toggle=root.querySelector('.hpc-primary-enabled input');if(toggle)toggle.checked=true;});
document.addEventListener('click',function(event){var button=event.target.closest('.hpc-primary-save');if(!button)return;var root=button.closest('[data-hpc-primary-entity]');var source=sourceValue(root);var search=root.querySelector('.hpc-primary-source-search[data-source-id="'+CSS.escape(source)+'"]');var objectId=search?search.querySelector('.hpc-smart-search-value').value:'';var status=root.querySelector('.hpc-primary-status');var body=new URLSearchParams();body.set('action',root.dataset.action||'');body.set(root.dataset.nonceField||'nonce',root.dataset.nonce||'');body.set('site_type',root.querySelector('.hpc-primary-site-type').value||'');body.set('enabled',root.querySelector('.hpc-primary-enabled input').checked?'1':'0');body.set('source',source);body.set('object_id',objectId);body.set('entity_type',(root.querySelector('.hpc-primary-derived-type')||{}).dataset?.value||'auto');root.classList.add('is-saving');if(status)status.textContent='Saving...';fetch(root.dataset.ajaxUrl||window.ajaxurl,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},body:body.toString()}).then(function(response){return response.json()}).then(function(payload){if(!payload||!payload.success)throw new Error(payload&&payload.data&&payload.data.message?payload.data.message:'Save failed');if(payload.data&&typeof payload.data.preview_html==='string'){var slot=root.querySelector('[data-hpc-primary-preview]');if(slot){slot.innerHTML=payload.data.preview_html;if(window.hexaPluginCoreInitPersistentDetails)window.hexaPluginCoreInitPersistentDetails(slot);}}if(status)status.textContent=payload.data.message||'Saved.';}).catch(function(error){if(status)status.textContent=error.message||'Unable to save.';}).finally(function(){root.classList.remove('is-saving')});});
