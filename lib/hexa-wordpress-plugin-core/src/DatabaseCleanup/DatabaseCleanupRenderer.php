...
final class DatabaseCleanupRenderer {
    ...
    private function styles( string $root_id ): void {
        ...
        <style>
            ...
            #<?php echo esc_attr( $root_id ); ?> .hpc-db-table-wrap{max-width:100%;overflow:auto}
            #<?php echo esc_attr( $root_id ); ?> .hpc-db-table{border-collapse:collapse;min-width:760px;width:100%}
            #<?php echo esc_attr( $root_id ); ?> .hpc-db-table th,#<?php echo esc_attr( $root_id ); ?> .hpc-db-table td{border-top:1px solid var(--hpc-line);font-size:12px;padding:10px;text-align:left;vertical-align:middle}
            ...
            #<?php echo esc_attr( $root_id ); ?> .hpc-db-state{align-items:center;display:inline-flex;gap:7px;font-weight:800}
    ...
    private function script( string $root_id ): void {
        ...
        (function(){
            ...
            function taskRow(t){return '<tr data-db-task-id="'+esc(t.id)+'"><td><strong>'+esc(t.label||t.id)+'</strong></td><td data-db-task-status>'+stateHtml('pending','Pending')+'</td><td data-db-task-result class="hpc-db-muted">Queued</td></tr>'}
