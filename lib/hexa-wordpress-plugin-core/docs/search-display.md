...
The consuming plugin must call `SearchDisplayRenderer::render()` for both each admin preview and the live shortcode. It must not copy the renderer markup, CSS, SVG, or JavaScript.
...
Supported renderer arguments are `style`, `accent`, `placeholder`, `action_url`, `label`, `radius`, and `id`.

## SearchDisplay Versus SmartSearch
