...
function run_login_status(): array {
    ...
    return [
        ...
        'message' => ! $class_exists
            ? 'Login masking is unavailable because its runtime class is not loaded.'
            : ( ! empty( $opts['enabled'] )
                ? 'Login masking is active. Current login URL: ' . Login_Masking::login_url()
                : 'Login masking is currently disabled.' ),
        'masking' => [
