...
use HWS\BaseTools\QuickStart\QuickStartBatchService;
...
class Dashboard_Config {
    ...
    public static function are_secret_urls_enabled() {
        return RemoteActionPolicy::option_enabled( self::OPT_SECRET_URLS_ENABLED );
    }
    ...
    public static function is_secret_setup_enabled() {
        return RemoteActionPolicy::option_enabled( self::OPT_SECRET_SETUP_ENABLED );
    }
    ...
    public static function is_secret_permalinks_enabled() {
        return RemoteActionPolicy::option_enabled( self::OPT_SECRET_PERMALINKS_ENABLED );
    }
}
...
function hws_get_overview_state_payload(): array {
    $remote_routes_enabled = Dashboard_Config::are_secret_urls_enabled()
        || Dashboard_Config::is_secret_setup_enabled()
        || Dashboard_Config::is_secret_permalinks_enabled();
    $secret = $remote_routes_enabled ? Dashboard_Config::get_secret_key() : '';
...
function hws_get_dashboard_tabs(): array {
    return DashboardRegistry::instance()->navigation_tabs();
}
...
function hws_normalize_dashboard_tab_id( string $tab_id ): string {
    return DashboardRegistry::instance()->normalize( $tab_id );
}
...
function ajax_load_dashboard_tab() {
    ...
    wp_send_json_success( [
        'tab'   => $tab_id,
        'label' => wp_strip_all_tags( DashboardRegistry::instance()->label( $tab_id ) ),
        'html'  => $html,
    ] );
}

function hws_render_dashboard_tab( string $tab_id ): void {
    if ( ! DashboardRegistry::instance()->render( $tab_id ) ) {
        echo '<div class="notice notice-error"><p>This HWS Base Tools section could not be loaded.</p></div>';
    }
}
...
function display_wp_admin_settings_page() {
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).on('click', '#hws-save-master-secret', function() {
            ...
            if (secret.length < 16) {
                $status.html('<span style="color:#d63638;">Secret must be at least 16 characters.</span>');
                return;
            }
            ...
            $.post(ajaxurl, {
                ...
                $btn.prop('disabled', false).text('Replace Secret');
                if (response.success) {
                    $('#hws-master-secret').val('');
...
function render_tab_overview() {
    ...
    $legacy_routes_on = $secret_urls_enabled || $secret_setup_enabled || $secret_permalinks_enabled;
    $secret = $legacy_routes_on ? Dashboard_Config::get_secret_key() : '';
    $debug_url = $secret_urls_enabled ? add_query_arg( Dashboard_Config::SECRET_DEBUG_KEY, $secret, home_url( '/' ) ) : '';
    $fatal_url = $secret_urls_enabled ? add_query_arg( Dashboard_Config::SECRET_FATAL_KEY, $secret, home_url( '/' ) ) : '';
    $setup_url = $secret_setup_enabled ? add_query_arg( Dashboard_Config::SECRET_SETUP_KEY, $secret, home_url( '/' ) ) : '';
    $permalinks_url = $secret_permalinks_enabled ? add_query_arg( Dashboard_Config::SECRET_PERMALINKS_KEY, $secret, home_url( '/' ) ) : '';
    ...
    <div class="hws-panel">
        ...
        <div class="hws-panel-body">
            ...
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                <input type="password" id="hws-master-secret" value="" autocomplete="new-password" placeholder="Enter a replacement secret" class="regular-text" style="font-family:monospace;font-size:14px;">
                <button type="button" id="hws-save-master-secret" class="hws-btn" style="white-space:nowrap;">Replace Secret</button>
            </div>
            <div id="hws-master-secret-status" style="font-size:13px;"></div>
            <p style="font-size:12px;color:#646970;margin:8px 0 0;">The stored value is encrypted and is never rendered back into this page. Legacy public routes remain disabled unless explicitly allowed in server configuration.</p>
        </div>
    </div>
    <!-- Debug Controls -->
    <div class="hws-panel">
...
function hws_execute_quick_setup( array $inputs = [] ) {
    // Compatibility entry point for callers that predate the v13 checklist.
    // Individual-only Review Center actions and mail sends never run here.
    return esc_html( ( new QuickStartBatchService() )->text_log() );
}
...
function ajax_save_master_secret() {
    ...
    if ( strlen( $new_secret ) < 16 ) {
        wp_send_json_error( 'Secret must be at least 16 characters' );
    }

    if ( ! hws_set_master_secret( $new_secret ) ) {
        wp_send_json_error( [ 'message' => 'The secret could not be encrypted and stored.' ] );
    }

    wp_send_json_success( [
        'message' => 'Master secret replaced successfully',
    ] );
}
