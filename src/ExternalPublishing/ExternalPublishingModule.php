...
final class ExternalPublishingModule implements ModuleInterface {
    ...
    public function can_use_collection(): bool|\WP_Error {
        ...
        return current_user_can( 'edit_posts' ) ? true : new \WP_Error( 'hws_external_publishing_forbidden', 'This WordPress user cannot create posts.', [ 'status' => 403 ] );
    }
    ...
    public function can_use_post( \WP_REST_Request $request ): bool|\WP_Error {
        ...
        return $post_id > 0 && current_user_can( $capability, $post_id ) ? true : new \WP_Error( 'hws_external_publishing_forbidden', 'This WordPress user cannot access the requested post.', [ 'status' => 403 ] );
    }
    ...
    public static function render(): void {
        ...
        $notice = isset( $_GET['hws_external_publishing_saved'] );
        if ( $notice ) {
            echo '<div class="notice notice-success inline"><p>External publishing setting saved.</p></div>';
        }
        echo '<div class="hws-section"><h2>External Publishing API</h2><p>Allows authenticated post submissions through HWS Base Tools using a WordPress Application Password. It is disabled by default.</p>';
    ...
    public static function save_settings(): void {
        ...
        update_option( self::OPTION, isset( $_POST['enabled'] ) ? 1 : 0, false );
