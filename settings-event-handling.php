...
// 2) Toggle snippet function with support for toggle- prefix on IDs
window.hws_base_tools.toggleSnippet = function(snippetId) {
  // Check for element with toggle- prefix first (new format), then raw ID (legacy)
  var $checkbox = jQuery('#toggle-' + snippetId);
  if ($checkbox.length === 0) {
    $checkbox = jQuery('#' + snippetId);
  }
  
  var isChecked = $checkbox.prop('checked');

  // Make the AJAX call to toggle the snippet
  jQuery.ajax({
    url: ajaxurl,
    type: 'post',
    data: {
    ...
    success: function(response) {
      if (response.success) {
        ...
        console.log('Snippet toggled: ' + snippetId + ' = ' + (isChecked ? 'enabled' : 'disabled'));
      } else {
